...
- 超时等待的线程:到一定时间会自动唤醒,不会等待
- 无限期等待的线程:不会自动唤醒,需要别人来唤醒
- 阻塞状态:当前线程遇到了锁,被阻塞住
- 结束状态:
...
- 有值的话就判断是否存在当前key并判断其属于链表节点还是Tree节点,然后进行值的更新,否则就相应的插入到对应的节点当中,如果是链表节点的8,如果节点个数到8,就转为Tree节点
...
- 校验key和value不能为空
- 同样是计算key的hashCode,然后找到其对应的数组下标,如果为空,就通过cas将该元素添加到集合中
- 然后判断当前元素是否正处于扩容阶段,如果处于扩容阶段,那么就进行协助扩容
- 否则锁定头节点,然后判断当前节点属于链表节点还是树节点,进行响应的插入操作,如果存在当前key,就将val值替换,并返回之前的值。
...
- 现在的CPU都是有缓存的,首先从主内存中将数据读到CPU缓存中,然后在到内存中去执行。volatile强制每次写数据的时候,都刷新到主内存,读的时候也要从主内存中读取。
...
- 轻量级锁:某个线程去获取锁的时候,将锁对象的MarkWord复制到本线程的锁记录上,然后将锁记录中的MarkWork中的锁记录指针通过CAS指向当前线程的锁记录,如果设置成功,那么代表获取到了轻量级锁,如果没有获取到,
